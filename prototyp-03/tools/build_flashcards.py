...
Pipeline, per card:
    ...
    tagged .tex source
        ...
        -> render a standalone 2-page a6-landscape PDF (page 1 = front,
           page 2 = back) reusing the client's existing preamble
        -> rasterize each page, trim the whitespace margin, save as .webp
        -> emit assets/flashcards_manifest.json

Re-runs are incremental: a card is only recompiled if its own LaTeX content
changed, tracked via a content hash cached in .build_cache.json. Adding one
new card to a 130-card deck re-renders one card, not 130.

Requirements (all external, not pip-installable):
    - a LaTeX toolchain with `latexmk` (or `pdflatex`) on PATH, able to
      compile the client's existing praeambel_karteikarten.tex
    - poppler-utils (`pdftoppm`) for PDF -> PNG rasterization
    - ImageMagick (`convert`) for autocropping the whitespace margin
...
def parse_source(tex_path: Path) -> tuple[str, list[CardSource]]:
    ...
    and including \\begin{document}, reused verbatim for every card so
    custom macros/environments from praeambel_karteikarten keep working."""
...
def render_card(
    ...
    # Bring along everything the preamble/includegraphics calls might need
    # (praeambel_karteikarten.tex, schema_*.pdf/png, custom .sty files, ...)
...
def pdf_page_to_webp(pdf_path: Path, page: int, out_path: Path, dpi: int = 400) -> None:
    """Rasterizes one page, trims the surrounding whitespace, and encodes
    as lossless webp (text/formula renders are small and benefit from
    exact edges more than they benefit from lossy compression)."""
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not rendered:
            raise RuntimeError(
                f"pdftoppm produced no output for {pdf_path} page {page}"
            )
        raw_png = rendered[0]

        trimmed_png = tmp_path / "trimmed.png"
        subprocess.run(
            [
                "convert",
                str(raw_png),
        ...
        subprocess.run(
            ["cwebp", "-lossless", "-z", "9", str(trimmed_png), "-o", str(out_path)],
            check=True,
            capture_output=True,
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory containing praeambel_karteikarten.tex "
        "and any images it includes (schema_*.pdf etc.)",
    )
    parser.add_argument(
        "--out-assets",
        required=True,
    ...
    parser.add_argument(
        "--force", action="store_true", help="Recompile every card, ignoring the cache"
    )
    args = parser.parse_args()

    for tool in ("latexmk", "pdftoppm", "convert", "cwebp"):
    ...
    print(
        f"Parsed {len(cards)} cards "
        f"({sum(1 for c in cards if c.category == 'skip')} skipped)."
    )

    flashcards_dir = args.out_assets / "flashcards"
    cache_path = args.out_assets.parent / ".build_cache.json"
    ...
    try:
        for card in cards:
            front_path = (
                flashcards_dir / card.category.lower() / f"{card.card_id}_front.webp"
            )
            back_path = (
                flashcards_dir / card.category.lower() / f"{card.card_id}_back.webp"
            )
    ...
    print(
        f"Rendered {rendered_count} card(s), {len(cards) - rendered_count} unchanged."
    )
